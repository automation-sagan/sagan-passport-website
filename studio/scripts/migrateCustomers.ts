/**
 * One-off migration: download the customer-vertical images from Framer, upload
 * them as Sanity assets, and create/replace one `customer` document per vertical
 * (id "customer.<slug>") with all the text + uploaded image references.
 *
 * Multi-entry: 7 documents, one per vertical. Safe to re-run — each vertical has
 * a fixed _id so createOrReplace overwrites that one document.
 *
 * Run from the studio dir (after `sanity login`):
 *   npx sanity exec scripts/migrateCustomers.ts --with-user-token
 */
...
// ---- source content (verbatim from web/src/data/customers.ts) --------------
interface ServiceCard { title: string; body: string }
interface TalentPerson { role: string; from: string; rate: string }
interface RoleEntry { title: string; country: string; rate: string }
interface Testimonial { image: string; quote: string; name: string; title: string }
...
const homeServices: CustomerData = {
...
const privateEquity: CustomerData = {
...
const smallBusinesses: CustomerData = {
...
const healthWellness: CustomerData = {
...
const professionalServices: CustomerData = {
...
const VERTICALS: {slug: string; data: CustomerData}[] = [
  {slug: 'home-services', data: homeServices},
  {slug: 'private-equity', data: privateEquity},
  {slug: 'small-businesses', data: smallBusinesses},
  {slug: 'health-wellness', data: healthWellness},
  {slug: 'franchises', data: franchises},
  {slug: 'professional-services', data: professionalServices},
  {slug: 'real-estate', data: realEstate},
]

// ---- build + write ---------------------------------------------------------
async function buildDoc(slug: string, d: CustomerData) {
  const heroImage = await uploadImage(d.hero.image)

  // Drop empty logo slots, then upload (dedup means the shared set uploads once).
  const logos = []
  for (const url of d.logos.filter(Boolean)) logos.push(await uploadImage(url))

  const caseStudyImage = await uploadImage(d.caseStudy.image)
  ...
  return {
    // Dot-free id: the dataset's anonymous read grant is `_id in path("*")`,
    // which only matches single-segment (dot-free) ids. A dotted id like
    // `customer.<slug>` would be unreadable by the tokenless build.
    _id: `customer-${slug}`,
    _type: 'customer',
    title: d.eyebrow,
    slug: {_type: 'slug', current: slug},
    seoTitle: `${d.eyebrow} — Sagan Passport`,
    eyebrow: d.eyebrow,
    hero: {title: d.hero.title, body: d.hero.body, image: heroImage},
    logosHeading: d.logosHeading,
    logos: withKeys(logos),
    serviceCards: withKeys(d.serviceCards.map((c) => ({_type: 'serviceCard', ...c}))),
    talent: {
      heading: d.talent.heading,
      subheading: d.talent.subheading,
      people: withKeys(d.talent.people.map((p) => ({_type: 'talentPerson', ...p}))),
      ticker1: d.talent.ticker1,
      ticker2: d.talent.ticker2,
    },
    caseStudy: {title: d.caseStudy.title, body: d.caseStudy.body, image: caseStudyImage, youtubeId: d.caseStudy.youtubeId},
    roles: {
      heading: d.roles.heading,
      body: d.roles.body,
      entries: withKeys(d.roles.entries.map((e) => ({_type: 'roleEntry', ...e}))),
    },
...
async function main() {
  // Remove any stale dotted-id docs from the earlier (unreadable) id scheme.
  const staleIds = VERTICALS.map(({slug}) => `customer.${slug}`)
  await client.delete({query: '*[_id in $ids]', params: {ids: staleIds}})

  for (const {slug, data} of VERTICALS) {
    console.log(`\nBuilding customer.${slug}…`)
    const doc = await buildDoc(slug, data)
    const res = await client.createOrReplace(doc)
    console.log(`✓ customer.${slug} written. _rev=${res._rev}`)
  }
  const count = await client.fetch('count(*[_type == "customer"])')
  console.log(`\n✓ Done. customer docs now in dataset: ${count}`)
}
